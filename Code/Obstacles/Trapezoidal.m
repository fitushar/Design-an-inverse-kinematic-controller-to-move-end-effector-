...
dx  = zeros(n,1); %velocity
ddx = zeros(n,1); % Acceleration


% repeat for each of the joint
for i=1:n
    delta = pos_f(i) - pos_i(i); %Distance
    dx_c(i) = sign(delta)*abs(dx_c(i));
    if (delta==0) % mean at final postion
        x(i)   = pos_i(i);
        dx(i)  = 0;
        ddx(i) = 0;
    else
        ...
        t_c = tf - delta/dx_c(i);
        % evaluates ddq_c
        ddq_c = dx_c(i)/t_c; % acceleration
        % if on the time slots
        if (t<=t_c) % Acceleration phase
            x(i)   = pos_i(i) + 0.5*ddq_c*t^2;
            dx(i)  = ddq_c*t;
            ddx(i) = ddq_c;
        elseif (t<=(tf-t_c)) %Constant phate
            x(i)   = pos_i(i) + dx_c(i)*(t-0.5*t_c);
            dx(i)  = dx_c(i);
            ddx(i) = 0;
        elseif (t<=tf)% Decelaration phase
