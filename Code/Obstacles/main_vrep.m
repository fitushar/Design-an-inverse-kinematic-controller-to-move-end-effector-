...
ee_vel_cruize   = [1.5 ,  1.5 ,  1.5 ]' .* abs(ee_pos_final - ee_pos_initial) / tf; % Maximum Velocity

obstacle_center = [-.55, 0, 0.2]';
%obstacle_center = [0.55 0 0.2]';
%obstacle_center = [0.7 0.7 0.7]';
...
pos_trpz        = zeros(3,N); % Position of Trapizoidal 
vel_trpz        = zeros(3,N); % Velocity of trapizoidal
acc_trpz        = zeros(3,N); % Accelation of trapizoidal
...
for i=1:N
    ...
    %If critical point of q is in the surface of influence then add
    %redundancy motion
    T = DirectKinematics(DH);
    n = size(DH,1);
    minimum = 10;
    ...
    for k = 1:n-1
        x_k1 = T(1:3,4,k);
        x_k2 = T(1:3,4,k+1);
        u = (x_k2 - x_k1) / sqrt(sum((x_k2 - x_k1).^2));
        a = u' * (obstacle_center - x_k1);
        x_c = x_k1 + a*u;
        d = sqrt(sum((x_c - obstacle_center).^2));
        if( obstacle_radius > d )
            obstacle = k;
        end
        if(d < minimum)
            w_current = x_c - obstacle_center;
            minimum = d;
        end
    end
    if obstacle > 0
        ...
        %             eye(size(J,2));
        %             pinv(J)*J;
        %             eye(size(J,2)) - pinv(J)*J;
        %             q_dot(:,i)
        q_dot(:,i) = q_dot(:,i) + ( eye(size(J,2)) - pinv(J)*J ) * qa_dot;
    end
